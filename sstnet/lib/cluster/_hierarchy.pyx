...
def nn_chain(double[:] dists, int n, int method, int[:] observation):
    ...
    method : int
        ...
        4: median 5: ward 6: weighted
    observation : ndarray
        A vector stores the number of the observations.
    ...
    cdef double[:] D = dists.copy()  # Distances between clusters.
    cdef int[:] size = observation.copy()  # number of the observations
    # cdef int[:] size = np.ones(n, dtype=np.intc)  # Sizes of clusters.
